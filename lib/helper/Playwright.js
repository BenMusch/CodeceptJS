...
} = require("./extras/PlaywrightReactVueLocator");
...
} = require("./extras/PlaywrightPropEngine");
const {
  seeElementError,
  dontSeeElementError,
...
class Playwright extends Helper {
  ...
  _setConfig(config) {
    ...
    if (this.options.video) {
      this.options.recordVideo = {
        size: parseWindowSize(this.options.windowSize),
      };
    }
  ...
  async _init() {
    ...
    try {
      ...
      if (process.env.testIdAttribute)
        await playwright.selectors.setTestIdAttribute(
          process.env.testIdAttribute
        );
    } catch (e) {
      console.warn(e);
    }
  ...
  _beforeSuite() {
    if (
      (restartsSession() || restartsContext()) &&
      !this.options.manualStart &&
      !this.isRunning
    ) {
  ...
  async _before(test) {
    ...
    if (restartsBrowser() && !this.options.manualStart)
    ...
    if (this.isElectron) {
      ...
      if (this.options.recordHar) {
        const harExt =
          this.options.recordHar.content &&
          this.options.recordHar.content === "attach"
            ? "zip"
            : "har";
        const fileName = `${`${global.output_dir}${path.sep}har${
          path.sep
        }${uuidv4()}_${clearString(this.currentRunningTest.title)}`.slice(
          0,
          245
        )}.${harExt}`;
      ...
      if (!this.browserContext || !restartsSession()) {
        this.browserContext = await this.browser.newContext(
          this.contextOptions
        ); // Adding the HTTPSError ignore in the context so that we can ignore those errors
      }
    }
    ...
    if (this.isElectron) {
      ...
      try {
        ...
        if (this.playwrightOptions.userDataDir) {
          this.browser = await playwright[
            this.options.browser
          ].launchPersistentContext(this.userDataDir, this.playwrightOptions);
          this.browserContext = this.browser;
          const existingPages = await this.browserContext.pages();
  ...
  async _after() {
    ...
    try {
      if ((await this.browser)._type === "Browser") {
        ...
        if (
          currentContext &&
          (this.options.keepCookies || this.options.keepBrowserState)
        ) {
  ...
  _session() {
    ...
    return {
      start: async (sessionName = "", config) => {
        this.debugSection(
          "New Context",
          config ? JSON.stringify(config) : "opened"
        );
        ...
        let page;
        if (this.isElectron) {
          const browser = await playwright._electron.launch(
            this.playwrightOptions
          );
          ...
          try {
            browserContext = await this.browser.newContext(
              Object.assign(this.contextOptions, config)
            );
            page = await browserContext.newPage();
          } catch (e) {
            if (this.playwrightOptions.userDataDir) {
              browserContext = await playwright[
                this.options.browser
  ...
  async resizeWindow(width, height) {
    if (width === "maximize") {
      throw new Error(
        "Playwright can't control windows, so it can't maximize it"
      );
    }
  ...
  async dragAndDrop(srcElement, destElement, options) {
    ...
    if (options) {
      return this.page.dragAndDrop(
        buildLocatorString(src),
        buildLocatorString(dst),
        options
      );
    }
    ...
    const destElBox = await this.page
      .locator(buildLocatorString(dst))
      .boundingBox();
  ...
  async scrollTo(locator, offsetX = 0, offsetY = 0) {
    ...
    if (locator) {
      ...
      await this.executeScript(
        (offsetX, offsetY) => window.scrollBy(offsetX, offsetY),
        {
      ...
      await this.executeScript(
        ({ offsetX, offsetY }) => window.scrollTo(offsetX, offsetY),
        { offsetX, offsetY }
      );
    }
    return this._waitForAction();
  ...
  async seeElement(locator) {
    ...
    try {
      return empty("visible elements").negate(
        els.filter((v) => v).fill("ELEMENT")
      );
    } catch (e) {
  ...
  async dontSeeElement(locator) {
    ...
    els = await Promise.all(els.map((el) => el.isVisible()));
    try {
      return empty("visible elements").assert(
        els.filter((v) => v).fill("ELEMENT")
      );
    } catch (e) {
  ...
  async seeElementInDOM(locator) {
    ...
    try {
      return empty("elements on page").negate(
        els.filter((v) => v).fill("ELEMENT")
      );
    } catch (e) {
  ...
  async dontSeeElementInDOM(locator) {
    const els = await this._locate(locator);
    try {
      return empty("elements on a page").assert(
        els.filter((v) => v).fill("ELEMENT")
      );
    } catch (e) {
  ...
  async selectOption(select, option) {
    ...
    try {
      optionToSelect = (
        await el.locator("option", { hasText: option }).textContent()
      ).trim();
    } catch (e) {
  ...
  async seeNumberOfElements(locator, num) {
    ...
    return equals(
      `expected number of elements (${new Locator(
        locator
      )}) is ${num}, but found ${elements.length}`
  ...
  async seeNumberOfVisibleElements(locator, num) {
    ...
    return equals(
      `expected number of visible elements (${new Locator(
        locator
      )}) is ${num}, but found ${res}`
    ).assert(res, num);
  }
  ...
  async seeCookie(name) {
    ...
    empty(`cookie ${name} to be set`).negate(
  ...
  async dontSeeCookie(name) {
    const cookies = await this.browserContext.cookies();
    empty(`cookie ${name} to be set`).assert(
      cookies.filter((c) => c.name === name)
    );
  }
  ...
  _contextLocator(locator) {
    ...
    if (this.contextLocator) {
      const contextLocator = buildLocatorString(
        new Locator(this.contextLocator, "css")
      );
  ...
  async grabCssPropertyFromAll(locator, cssProperty) {
    ...
    const cssValues = await Promise.all(
      els.map((el) =>
        el.evaluate(
          (el, cssProperty) =>
            getComputedStyle(el).getPropertyValue(cssProperty),
          cssProperty
        )
      )
    );
  ...
  async seeCssPropertiesOnElements(locator, cssProperties) {
    ...
    const cssPropertiesCamelCase =
      convertCssPropertiesToCamelCase(cssProperties);
    ...
    return equals(
      `all elements (${new Locator(
        locator
      )}) to have CSS property ${JSON.stringify(cssProperties)}`
  ...
  async seeAttributesOnElements(locator, attributes) {
    ...
    res.forEach((el) => {
      Object.keys(attributes).forEach((prop) => {
        commands.push(
          el.evaluate((el, attr) => el[attr] || el.getAttribute(attr), prop)
        );
      });
    });
    ...
    return equals(
      `all elements (${new Locator(
        locator
      )}) to have attributes ${JSON.stringify(attributes)}`
    ).assert(chunked.length, elemAmount);
  }
  ...
  async dragSlider(locator, offsetX = 0) {
    ...
    // Drag destination
    await this.page.mouse.move(sliderSource.x + offsetX, sliderSource.y, {
      steps: 5,
    });
    await this.page.mouse.up();
  ...
  async saveElementScreenshot(locator, fileName) {
    ...
    const elem = res;
    this.debug(
      `Screenshot of ${new Locator(
        locator
      )} element has been saved to ${outputFile}`
    );
  ...
  async _failed(test) {
    ...
    const uuid =
      this.currentRunningTest.uuid ||
      (this.currentRunningTest.ctx && this.currentRunningTest.ctx.uuid) ||
      uuidv4();

    if (this.options.recordVideo && this.page && this.page.video()) {
      test.artifacts.video = await saveVideoForPage(
        this.page,
        `${test.title}.failed`,
        uuid
      );
  ...
  async waitForEnabled(locator, sec) {
    ...
    if (!locator.isXPath()) {
      ...
      waiter = context.waitForFunction(valueFn, [locator.value], {
        timeout: waitTimeout,
      });
    } else {
      ...
      waiter = context.waitForFunction(
        enabledFn,
        [locator.value, $XPath.toString()],
        { timeout: waitTimeout }
      );
    }
  ...
  async waitForValue(field, value, sec) {
    ...
    if (!locator.isXPath()) {
      ...
      waiter = context.waitForFunction(
        valueFn,
        [locator.value, $XPath.toString(), value],
        { timeout: waitTimeout }
      );
    }
    return waiter.catch((err) => {
  ...
  async waitNumberOfVisibleElements(locator, num, sec) {
    ...
    if (locator.isCSS()) {
      const visibleFn = function ([locator, num]) {
        ...
        return (
          Array.prototype.filter.call(els, (el) => el.offsetParent !== null)
            .length === num
        );
      };
      ...
      const visibleFn = function ([locator, $XPath, num]) {
        eval($XPath); // eslint-disable-line no-eval
        return (
          $XPath(null, locator).filter((el) => el.offsetParent !== null)
            .length === num
        );
      };
      waiter = context.waitForFunction(
        visibleFn,
        [locator.value, $XPath.toString(), num],
        { timeout: waitTimeout }
      );
    }
    return waiter.catch((err) => {
  ...
  async waitForInvisible(locator, sec) {
    ...
    if (this.frame) {
      ...
      if (!waiter)
        throw new Error(
          `element (${locator.toString()}) still visible after ${
            waitTimeout / 1000
          } sec.`
        );
      return;
  ...
  async waitForNumberOfTabs(expectedTabs, sec) {
    ...
    throw new Error(
      `Expected ${expectedTabs} tabs are not met after ${
        waitTimeout / 1000
      } sec.`
    );
  }
  ...
  async waitForText(text, sec = null, context = null) {
    const waitTimeout = sec ? sec * 1000 : this.options.waitForTimeout;
    const errorMessage = `Text "${text}" was not found on page after ${
      waitTimeout / 1000
    } sec.`;
    let waiter;
  ...
  async switchTo(locator) {
    if (Number.isInteger(locator)) {
      ...
      if (locator >= 0 && locator < childFrames.length) {
        ...
        throw new Error(
          "Element #invalidIframeSelector was not found by text|CSS|XPath"
        );
      }
      return;
    }
    ...
    if (!frame) {
      throw new Error(
        `Frame ${JSON.stringify(locator)} was not found by text|CSS|XPath`
      );
    }
  ...
  async waitForDetached(locator, sec) {
    ...
    if (!locator.isXPath()) {
      try {
        ...
        throw new Error(
          `element (${locator.toString()}) still on page after ${
            waitTimeout / 1000
          } sec\n${e.message}`
        );
      }
    ...
    return promiseRetry(
      async (retry, number) => {
        const _grabCookie = async (name) => {
          ...
          const cookie = cookies.filter((c) => c.name === name);
          if (cookie.length === 0)
            throw Error(`Cookie ${name} is not found after ${retries}s`);
        };
...
async function proceedClick(locator, context = null, options = {}) {
  ...
  */
  if (options.force) {
    ...
    const element =
      els.length > 1 ? (await getVisibleElements(els))[0] : els[0];
    await element.click(options);
  }
  const promises = [];
  if (options.waitForNavigation) {
    promises.push(
      this.waitForURL(/.*/, { waitUntil: options.waitForNavigation })
    );
  }
...
async function findClickable(matcher, locator) {
  ...
  els = await findElements.call(
    this,
    matcher,
    Locator.clickable.narrow(literal)
  );
  ...
  try {
    els = await findElements.call(
      this,
      matcher,
      Locator.clickable.self(literal)
    );
    if (els.length) return els;
  } catch (err) {
    // Do nothing
...
async function proceedSee(assertType, text, context, strict = false) {
  ...
  if (strict) {
    return allText.map((elText) =>
      equals(description)[assertType](text, elText)
    );
  }
...
async function findCheckable(locator, context) {
  ...
  let els = await findElements.call(
    this,
    contextEl,
    Locator.checkable.byText(literal)
  );
  if (els.length) {
    return els;
  }
  els = await findElements.call(
    this,
    contextEl,
    Locator.checkable.byName(literal)
  );
  if (els.length) {
    return els;
  }
  ...
  const proceedMultiple = async (elements) => {
    ...
    if (typeof value === "boolean") {
      ...
      if (assertType === "assert") {
        equals(`select option by ${field}`)[assertType](
          true,
...
async function refreshContextSession() {
  ...
  if (currentUrl.startsWith("http")) {
    await this.executeScript("localStorage.clear();").catch((err) => {
      if (
        !(err.message.indexOf("Storage is disabled inside 'data:' URLs.") > -1)
      )
        throw err;
    });
...
async function saveVideoForPage(page, name, uuid) {
  if (!page.video()) return null;
  const fileName = `${`${
    global.output_dir
  }${pathSeparator}videos${pathSeparator}${uuid}_${clearString(name)}`.slice(
    0,
    245
  )}.webm`;
  page
...
async function saveTraceForContext(context, name, uuid) {
  if (!context) return;
  if (!context.tracing) return;
  const fileName = `${`${
    global.output_dir
  }${pathSeparator}trace${pathSeparator}${uuid}_${clearString(name)}`.slice(
    0,
    245
  )}.zip`;
